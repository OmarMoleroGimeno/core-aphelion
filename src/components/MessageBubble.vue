...
<template>
  <div class="flex w-full my-6" :class="isUser ? 'justify-end' : 'justify-start'">
    <div 
      class="flex flex-col max-w-[85%] md:max-w-[70%]"
      :class="isUser ? 'items-end' : 'items-start'"
    >
      <!-- Sender Name (Optional) -->
      <span class="text-xs text-zinc-400 dark:text-zinc-200 mb-1 px-1">
      ...
      >
        ...
        <div 
          v-if="!isUser"
          class="markdown-content !text-base"
      ...
      <!-- Timestamp -->
      <span class="text-xs text-zinc-400 dark:text-zinc-200 mt-1 px-1 opacity-70">
        {{ timeString }}
      </span>
    </div>
  </div>
